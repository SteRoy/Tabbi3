                ...
                models.Debate.findOne({
                    ...
                    if (debate) {
                        if (debate.AdjAllocs.length > 0) {
                            req.adjudicator = debate.AdjAllocs.AdjudicatorId;
                            if (debate.Ballots.some(b => b.enteredByTab || b.finalised)) {
                                return res.status(401).json({error: `This Debate is no longer accepting e-Ballots.`});
                            } else {
                                next();
                            }
                        } else {
